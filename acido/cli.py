...
run_parser.add_argument('--expose-ip', dest='expose_ips', action='append',
                         help='Source IP address(es) allowed to access container through firewall. Creates NAT rules for each source IP/port combination. Use "*" for any source. Can be specified multiple times. Requires configured firewall and --expose-port.')
run_parser.add_argument('--dmz', dest='dmz', action='store_true',
                         help='DMZ mode: Allow all traffic on all ports to reach the container (10.0.2.4). Requires configured firewall.')
...
class Acido(object):
    ...
    def run(self, name: str, image_name: str, task: str = None, duration: int = 900,
            ...
            entrypoint: str = None, expose_ips: list = None, dmz: bool = False):
        """
        ...
        Args:
            ...
            expose_ips: List of source IP addresses allowed to access container via firewall (requires firewall and --expose-port)
            dmz: DMZ mode - allow all traffic on all ports to container (requires firewall)
        ...
        # Validate --expose-ip and --dmz requirements
        if expose_ips or dmz:
            # --expose-ip and --dmz require a configured firewall
            if not self.firewall_name or not self.firewall_public_ip:
                print(bad("--expose-ip and --dmz require a configured firewall"))
                print(info("Please create and configure a firewall first using 'acido firewall create'"))
                raise ValueError("--expose-ip and --dmz require a configured firewall")
        ...
        if dmz and expose_ips:
            print(bad("--dmz and --expose-ip cannot be used together (--dmz allows all IPs on all ports)"))
            raise ValueError("--dmz and --expose-ip cannot be used together")
        ...
        # Check if using firewall ingress mode (firewall configured and expose_ips/dmz set)
        if self.firewall_name and self.firewall_public_ip and (expose_ips or dmz):
            # Firewall + expose_ips/dmz mode: Use ingress subnet, inject firewall IP as env var
        ...
        elif bidirectional and not expose_ips and not dmz:
        ...
        # Automatic firewall rule creation when --expose-ip or --dmz are set
        # This is the new mode: firewall-based exposure with automatic rule creation
        if (expose_ips or dmz) and bidirectional and (exposed_ports or dmz):
            ...
            # Collect all ports from exposed_ports (or use wildcard for DMZ)
            if dmz:
                # DMZ mode: allow all ports
                port_numbers = ["*"]
                protocols = ["Any"]
            else:
            ...
            try:
                # Use timestamp to make network rule name unique
                import time
                timestamp = int(time.time())
                network_rule_name = f"{name}-outbound-{timestamp}"
            ...
            # Note: Destination address must be the firewall's public IP
            # Note: Source addresses are the IPs specified in --expose-ip (or "*" for any)
            container_private_ip = CONTAINER_DEFAULT_PRIVATE_IP  # First container in subnet
            
            if dmz:
                # DMZ mode: Create a single NAT rule for all traffic to container
                try:
                    nat_rule_name = f"{name}-dmz-all"
                    print_if_not_quiet(info(f"Creating DMZ NAT rule: {nat_rule_name}"))
                    
                    # DMZ rule: Allow all traffic from any source to firewall -> container
                    self.firewall_manager.create_dnat_rule(
                        firewall_name=self.firewall_name,
                        rule_collection_name="acido-auto-nat",
                        rule_name=nat_rule_name,
                        source_addresses=["*"],  # Allow from any source IP
                        destination_address=self.firewall_public_ip,  # Firewall's public IP
                        destination_port="*",  # All ports
                        translated_address=container_private_ip,
                        translated_port="*",  # All ports
                        protocol="Any"  # All protocols
                    )
                except Exception as e:
                    print_if_not_quiet(orange(f"DMZ NAT rule creation failed: {e}"))
                    print_if_not_quiet(info("Continuing with container deployment..."))
            else:
                # Standard mode: Create NAT rules for each source IP and port combination
                # Default to "*" (any source) if no expose_ips specified
                source_ips = expose_ips if expose_ips else ["*"]
                ...
                for source_ip in source_ips:
                    for port_spec in exposed_ports:
                        ...
                        try:
                            # Create unique rule name using source IP and port
                            # Replace dots and asterisks in source IP with dashes for valid rule name
                            ip_safe = source_ip.replace('.', '-').replace('*', 'any')
                            nat_rule_name = f"{name}-nat-{ip_safe}-{port}-{protocol.lower()}"
                            print_if_not_quiet(info(f"Creating NAT rule: {nat_rule_name}"))
                            
                            # Create DNAT rule: Firewall public IP:port -> Container private IP:port
                            # Source IP filtering applied via source_addresses
                            # Destination is always the firewall's public IP
                            self.firewall_manager.create_dnat_rule(
                                firewall_name=self.firewall_name,
                                rule_collection_name="acido-auto-nat",
                                rule_name=nat_rule_name,
                                source_addresses=[source_ip],  # Filter by source IP
                                destination_address=self.firewall_public_ip,  # Firewall's public IP
                            ...
                            print_if_not_quiet(orange(f"NAT rule creation failed for source {source_ip}:{port}/{protocol}: {e}"))
                            print_if_not_quiet(info("Continuing with next rule..."))
            
            print_if_not_quiet(good("Firewall rules created successfully"))
            # Show firewall access information
            if dmz:
                print_if_not_quiet(info(f"DMZ mode: Container accessible at {self.firewall_public_ip} on all ports from any source"))
            else:
                source_info = ', '.join(expose_ips) if expose_ips else 'any source'
                port_info = ', '.join([f"{p['port']}/{p['protocol']}" for p in exposed_ports])
                print_if_not_quiet(info(f"Container accessible at {self.firewall_public_ip} on ports {port_info} from {source_info}"))
